...
void CreateVertex(float dx, float dy, float dw, float dh, int tcx, int tcy, int tcw, int tch)
{
	...
	pV[0].pos = D3DXVECTOR4(dx, dy, 1.0f, 1.0f);
	pV[1].pos = D3DXVECTOR4(dx + dw, dy, 1.0f, 1.0f);
	pV[2].pos = D3DXVECTOR4(dx + dw, dy + dh, 1.0f, 1.0f);
	pV[3].pos = D3DXVECTOR4(dx, dy + dh, 1.0f, 1.0f);
...
void CreateVertexAffine(float dx, float dy, float dw, float dh, int tcx, int tcy, int tcw, int tch)
{
	...
	pV[0].pos = D3DXVECTOR4(dx+x  *cosf(g_fPolygonAngle) - y  *sinf(g_fPolygonAngle), dy+x  *sinf(g_fPolygonAngle) + y  *cosf(g_fPolygonAngle), 1.0f, 1.0f);
	pV[1].pos = D3DXVECTOR4(dx+x_w*cosf(g_fPolygonAngle) - y  *sinf(g_fPolygonAngle), dy+x_w*sinf(g_fPolygonAngle) + y  *cosf(g_fPolygonAngle), 1.0f, 1.0f);
	pV[2].pos = D3DXVECTOR4(dx+x_w*cosf(g_fPolygonAngle) - y_h*sinf(g_fPolygonAngle), dy+x_w*sinf(g_fPolygonAngle) + y_h*cosf(g_fPolygonAngle), 1.0f, 1.0f);
	pV[3].pos = D3DXVECTOR4(dx+x  *cosf(g_fPolygonAngle) - y_h*sinf(g_fPolygonAngle), dy+x  *sinf(g_fPolygonAngle) + y_h*cosf(g_fPolygonAngle), 1.0f, 1.0f);
